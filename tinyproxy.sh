...
function install_tinyproxy() {
  ...
  sudo cat <<EOF >/usr/local/etc/tinyproxy/tinyproxy.conf
##
...
#ErrorFile 502 "/usr/local/share/tinyproxy/502.html"
...
#ReverseBaseURL "http://localhost:8888/"
EOF
  systemctl start tinyproxy          #开启tinyproxy
  systemctl restart tinyproxy          #开启tinyproxy
  systemctl enable tinyproxy.service #设置开机自动启动
}
install_tinyproxy
